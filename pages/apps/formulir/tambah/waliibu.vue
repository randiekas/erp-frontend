<template>
	<div>
        <Head
            title="Biodata"
            subtitle="Isi Semua Data Orang Tua (Ibu Kandung)"
            color="text--black"/>
        <Form
            :fields="fields"
            :model="model"
...
export default {
    layout: 'apps',
    async asyncData({ $api }) {
		let model = (await $api.$get(`/api/v1/ppdb/ibu/detil/0`)).data
		return {
			model: model || {
                penghasilan: 0,
	...
	methods:{
		handleSimpan(){
			...
			this.$api.$post(`/api/v1/ppdb/ibu/simpan`, payload).then(async (resp)=>{
